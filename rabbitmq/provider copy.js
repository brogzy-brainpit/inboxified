...
const rabbitProvider=(amqp,subject,list,html)=>{
     const compileTemplate = Handlebars.compile(html); // <-- THIS is the key line
  ...
  return  new Promise((resolve, reject) => {
    ...
    const personalizedHtml = compileTemplate(contact); // <-- This replaces {{name}}, {{email}}, etc.
  }
                            sent++;                
                            sender({
                                from :'bymemet videos <dangabarin2020@gmail.com>',
                                to:list[sent-1].email,
                                replyTo:"dangabarin2020@gmail.com",
                                subject: subject + sent,
                                text: `Hey ${list[sent-1].name}, I came across your [Instagram/website] and noticed your coaching offer looks solid — but your site isn’t showing it off the way it could.
